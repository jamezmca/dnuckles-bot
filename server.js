...
const emails = ["gmail", "outlook", "hotmail", "live", "yahoo"]
...
app.get('/', async (req, res) => {
    ...
    const configuration = new Configuration({
        organization: "org-ctyeKq23VIkruQcaaoLxCd7Q",
        apiKey: process.env.OPENAI_API_KEY,
    })
    const openai = new OpenAIApi(configuration)
    const reviewTitle = await openai.createCompletion('text-davinci-001', {
        prompt: `without writing the name the restaurant, write a title for a breakfast restaurant review where the experience was satisfactory.`,
        max_tokens: 14
    })
    const reviewText = await openai.createCompletion('text-davinci-001', {
        prompt: `without writing the name the restaurant, write a review for a breakfast restaurant where the service was a bit cold, the food was a good, and the cost was average.`,
        max_tokens: 55
    })
    const randomBreakfast = await openai.createCompletion('text-davinci-001', {
        prompt: `Name three breakfast meals in 8 words or less.`,
        max_tokens: 10
    })
    ...
    try {
        ...
        const firstName = names[Math.floor(Math.random() * length)]
        const lastName = names[Math.floor(Math.random() * length)]
        const email = `${firstName}${lastName}${Math.floor(Math.random() * 100)}@${emails[Math.floor(Math.random() * emails.length)]}.com`
        ...
        await page.waitForSelector('#REVIEWS .block_header a')
        await page.click('#REVIEWS .block_header a')
        console.log('filling out review form')
        ...
        await page.mouse.click(coordinates.x + 75, coordinates.y + 5)
        await page.waitForTimeout(2000)

        await page.waitForSelector('span[data-name="Service"]')
        await page.type('#REVIEW_TITLE input#ReviewTitle', reviewTitle.data.choices[0].text, { delay: 80 })
        await page.type('#REVIEW_TEXT textarea#ReviewText', reviewText.data.choices[0].text, { delay: 80 })
        console.log('left food review')
        ...
        const mealTypes = await page.evaluate(() => (
            Array.from(document.querySelectorAll('#MEAL_TYPE option')).map(val => val.value).filter(val => val != '')
        ))
        await page.select('#MEAL_TYPE select', `${mealTypes[Math.floor(Math.random() * mealTypes.length)]}`)
        await page.waitForTimeout(1000)

        const visitDate = await page.evaluate(() => (
            Array.from(document.querySelectorAll('#DATE_OF_VISIT option')).map(val => val.value).filter(val => val != '')
        ))
        await page.select('#DATE_OF_VISIT select', `${visitDate[Math.floor(Math.random() * visitDate.length)]}`)
        await page.waitForTimeout(1000)
        console.log('left food review and meal type and visit date')
        await page.type('#EATERY_DISH input', randomBreakfast.data.choices[0].text, { delay: 80 })

        await page.evaluate(() => {
            const scrollableSection = document.querySelector('span[data-name="Value"]');
            scrollableSection.scrollTop = scrollableSection.offsetHeight;
        })
        await page.waitForTimeout(3000)

        const rO2 = await page.$('span[data-name="Service"]')
        const co2 = await rO2.boundingBox()
        await page.mouse.click(co2.x + (Math.floor(Math.random() * 30) + 40), co2.y + 5)
        await page.waitForTimeout(1000)


        const rO3 = await page.$('span[data-name="Food"]')
        const co3 = await rO3.boundingBox()
        await page.mouse.click(co3.x + (Math.floor(Math.random() * 30) + 40), co3.y + 5)
        await page.waitForTimeout(1000)

        const rO4 = await page.$('span[data-name="Value"]')
        const co4 = await rO4.boundingBox()
        await page.mouse.click(co4.x + (Math.floor(Math.random() * 30) + 40), co4.y + 5)
        await page.waitForTimeout(1000)

        await page.click(`#DQ_TAGRADIO div:nth-child(2) div:nth-child(${Math.floor(Math.random() * 3)})`)
        await page.waitForTimeout(5000)

        await page.click('input#noFraud')
        await page.waitForTimeout(1000)
        await page.click('#SUBMIT')
        await page.waitForTimeout(4000)

        await page.waitForSelector('.fGwNR._G.B-.z._S.c.Wc.ddFHE.fRPQK.w.ezIjy.brHeh')
        await page.click('.fGwNR._G.B-.z._S.c.Wc.ddFHE.fRPQK.w.ezIjy.brHeh')
